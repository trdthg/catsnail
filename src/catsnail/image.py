...
def _resolve_remote_iso(
    ...
    with lock_path.open("a+", encoding="utf-8") as lock:
        ...
        try:
            ...
            temporary = directory / f".image.{os.getpid()}.part"
            try:
                try:
...
def _download_iso(url: str, destination: Path) -> tuple[str, int]:
    ...
    size = 0
    request = Request(url, headers={"User-Agent": "catsnail/0.1"})
    with urlopen(request, timeout=60) as response, destination.open("wb") as output:  # noqa: S310 - user-declared ISO URL
